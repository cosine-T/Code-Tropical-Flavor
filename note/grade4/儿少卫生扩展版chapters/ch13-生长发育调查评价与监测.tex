...
\begin{enumerate}[leftmargin=*]
    \item \imp{发育水平（Level）}：反映个体或群体儿童在当前时间点的生长发育达到的程度。
    \item \imp{发育速度（Speed）}：反映生长发育指标的动态变化趋势。
    \item \imp{发育匀称度（Proportionality）}：反映身体各部分发育的协调性和比例关系。
\end{enumerate}

\textbf{参考标准类型：}

\begin{enumerate}[leftmargin=*]
    \item \imp{现状正常值（Status Quo Norm）}：样本来自本地区中等水平人群，仅剔除慢性疾患或残障对生长发育有明显不良影响的个体。反映本地区、本人群的真实情况，也是干预目标。
    \item \imp{理想正常值（Ideal Norm）}：对象均应为足月正常体重新生儿，自幼生活在适宜环境，营养充分，无慢性病史，有良好生活条件和卫生服务，生长潜力得以充分发挥。一旦建立，能长期使用。
    \item \imp{WHO生长标准（WHO Growth Standards）}：5岁以下儿童建议采用WHO生长标准；学龄期尤其是青春期开始后出现较大差异，中国相对略高的界值点百分位线可能利于早期筛查生长异常儿童。
\end{enumerate}
\end{defbox}
...
\textbf{1. 评价原理：}一种特殊类型的离差法，以0为中心，用偏离该年龄组标准差的程度反映生长情况。计算公式：$Z = (x - \bar{x}) / s$
...
\hline
\textbf{Z值} & $>2$ & $1 \sim 2$ & $-1 \sim 1$ & $-2 \sim -1$ & $<-2$ \\
\hline
\end{tabularx}
\end{table}
...
\begin{itemize}[leftmargin=*]
    \item \imp{优点}：消除了年龄、性别差异，可跨年龄、跨性别进行比较；计算结果标准化，便于统计分析。
    \item \imp{缺点}：要求资料呈正态分布；计算相对复杂。
\end{itemize}
\end{keybox}

\begin{tipbox}
\textbf{三种发育水平评价方法的选用原则：}

\begin{itemize}[leftmargin=*]
    \item 正态分布资料：三种方法均可使用，离差法和Z分法更为精确。
    \item 偏态分布资料：优选百分位数法。
    \item 国际比较或需要标准化时：选用Z分法（WHO推荐）。
    \item 大规模群体筛查：百分位数法最为常用。
\end{itemize}
\end{tipbox}
...
\begin{itemize}[leftmargin=*]
    \item 男性指标：睾丸容积、阴毛、喉结、变声。
    \item 女性指标：乳房、腋毛和阴毛。
    \item 适用范围：仅适用于青春期（初高中阶段）。
\end{itemize}
...
\textbf{（四）骨龄 / 骨骼年龄（Skeletal Age）\difficulty}
