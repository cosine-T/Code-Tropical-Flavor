...
\textbf{\color{keyred}本章教学难点：}食物热效应（TEF/SDA）\difficulty、能量需要量测定方法\difficulty。
}
...
\begin{keybox}
\textbf{食物热效应（TEF/SDA）}\difficulty：
...
\begin{keybox}
\textbf{体重判定标准——标准体重计算公式：}
\begin{itemize}[leftmargin=*]
    \item \textbf{Broca公式：}标准体重(kg) = 身高(cm) $-$ 100
    \item \textbf{Broca改良公式：}标准体重(kg) = 身高(cm) $-$ 105
    \item \textbf{平田公式：}标准体重(kg) = [身高(cm) $-$ 100] $\times$ 0.9
\end{itemize}

\textbf{体重判定：}
\begin{itemize}[leftmargin=*]
    \item 正常体重：为标准体重 $\pm$10\%
    \item 超重：体重超过标准体重的10\%
    \item 肥胖：体重超过标准体重的20\%
\end{itemize}

\textbf{体质指数（BMI）= 体重(kg) / 身高(m)$^2$}
\begin{itemize}[leftmargin=*]
    \item 中国标准：BMI $<$ 18.5 偏瘦，18.5～23.9 正常，24.0～27.9 超重，$\geq$ 28.0 肥胖
\end{itemize}
\end{keybox}
...
\textbf{科学减重原则：}
\begin{enumerate}[leftmargin=*]
    \item 减的是脂肪，而不是瘦体质（肌肉）
    \item 每周减重不超过1 kg
    \item 能量摄入每天不应低于1200 kcal（女性），最低不少于1000 kcal
    \item 每天至少保证三顿饭
    \item 供能比：蛋白质15\%～20\%，脂肪20\%～30\%
\end{enumerate}
\end{keybox}
