...
\textbf{理想甜味剂应具备的特点：}
\begin{enumerate}[leftmargin=*]
    \item \textbf{安全性好：}经过充分的毒理学评价，对人体无任何健康危害。
    \item \textbf{味觉良好：}甜味纯正，无不良后味或异味，口感接近蔗糖。
    \item \textbf{稳定性好：}对热、酸、碱等加工条件稳定，储存期间不分解变质。
    \item \textbf{水溶性好：}在水中溶解度大，便于在各种食品体系中均匀使用。
    \item \textbf{价格低廉：}生产成本合理，具有市场竞争力。
\end{enumerate}
\end{keybox}
...
\begin{enumerate}[leftmargin=*]
    \item \textbf{漂白剂（Bleaching Agent）：}能够破坏、抑制食品的发色因素，使其褪色或使食品免于褐变的物质。
    \begin{itemize}[leftmargin=*]
        \item \textbf{氧化性漂白剂：}主要用于面粉漂白（如过氧化苯甲酰，已禁用）。
        \item \textbf{还原性漂白剂：}常用品种包括二氧化硫、焦亚硫酸钾、焦亚硫酸钠、亚硫酸钠、亚硫酸氢钠、低亚硫酸钠、硫黄等。\keyword{二氧化硫既是漂白剂又是防腐剂}，主要用于表面处理的鲜水果、水果干类、蜜饯凉果、干制蔬菜、腌渍蔬菜、蔬菜罐头等。还原性漂白剂应注意SO$_2$残留量，过量可破坏食品中维生素B1。
    \end{itemize}
    \item \textbf{酶制剂（Enzyme Preparations）：}用动物或植物的可食或非可食部分直接提取，或用传统或通过基因修饰的微生物发酵提取制得的，具有特殊催化功能的生物制品。主要用于加速食品加工过程和提高食品产品质量。
    \begin{itemize}[leftmargin=*]
        \item \textbf{常用种类：}谷氨酰胺转移酶（改善蛋白质凝胶性）、木瓜蛋白酶（嫩化肉类）、$\alpha$-淀粉酶（淀粉糖化）。
    \end{itemize}
    \item \textbf{增味剂（Flavor Enhancer）：}可补充或增强食品原有风味的物质。增味剂可能本身并没有鲜味，但却能增加食物的天然鲜味。
    \begin{itemize}[leftmargin=*]
        \item \textbf{谷氨酸钠（MSG，味精）：}最常用的增味剂，赋予食品鲜味。
        \item \textbf{核苷酸系列增味剂：}5$'$-呈味核苷酸二钠（IMP+GMP），与MSG有强烈协同增鲜效应。
        \item \textbf{其他：}麦芽酚（具有焦糖香气）、酵母提取物（天然复合增味剂）。
    \end{itemize}
