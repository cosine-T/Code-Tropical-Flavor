...
| 文件类型 | 说明 | 用途 |
|---------|------|------|
| `大纲.doc` | 本学年课程教学大纲（.doc 格式） | 确定章节框架、掌握/熟悉/其他划分 |
| `*.pptx` | 教师授课PPT（每个章节一个文件），放在 `ref_ppt/` | 补充知识点 |
| `往届笔记.pdf` | 往届复习笔记或参考资料 | **优先参考源**，知识点主来源 |

### 步骤1.5：提取并保存所有参考文件文字（必须）

**在编写任何内容之前**，必须提取所有参考文件的文字并保存到参考目录下，供用户核查：

```bash
# DOC 大纲 → 大纲_提取文本.txt
antiword "XX参考/大纲.doc" > "XX参考/大纲_提取文本.txt"

# PDF 往届笔记 → 往届笔记_提取文本.txt
pdftotext -enc UTF-8 "XX参考/往届笔记.pdf" "XX参考/往届笔记_提取文本.txt"

# PPT 文件 → ppt_提取文本/ 子目录（每个PPT一个txt）
mkdir -p "XX参考/ppt_提取文本"
python3 -c "
from pptx import Presentation
import os, glob
for f in sorted(glob.glob('XX参考/ref_ppt/*.pptx')):
    prs = Presentation(f)
    out = os.path.join('XX参考/ppt_提取文本', os.path.basename(f).replace('.pptx','.txt'))
...
从大纲中提取：
- **章节列表**：确定笔记的章节目录（注意：大纲章节编号可能与参考PDF/PPT不同，需要做**章节映射**）
- **重点标记**（严格三级分类）：
  - 双下划线 = **掌握内容** → 写入正文"掌握"部分，用 `\keybox`（红色框）
  - 单下划线 = **熟悉内容** → 写入正文"熟悉"部分，用 `\defbox` 或详细正文
  - 无标记 = **其他内容** → 写入正文"其他"部分，用 `\tipbox` 或普通正文
  - 句尾 `*` = **教学难点** → 标题加 `\difficulty`，在章节导览和正文中标注
...
1. **框架优先**：以教学大纲的章节目录为骨架，**大纲章节号与参考材料可能不同，需手动映射**
2. **来源优先级**：往届笔记 > ref_ppt。笔记为主，PPT补充笔记缺失的内容
3. **严格引用原文**：所有知识点必须逐字引用参考材料原文，不得改写、转述或添加材料中没有的解释
4. **章节内按三级组织**：掌握内容 → 熟悉内容 → 其他内容，每部分用小节标题区分
5. **重难点突出**：
   - 大纲双下划线内容 → `\keybox`（红色框）+ `\keyword{}`
   - 大纲单下划线内容 → `\imp{}` + 正文详述
   - 大纲 `*` 难点 → 章节导览中注明 + 标题加 `\difficulty`
6. **所有定义写入正文**：往届笔记中每个有明确定义的概念都必须以 `\defbox` 或 `\keybox` 写入章节正文知识点中，不能只在复习题答案里出现
7. **包含往届笔记绝大部分内容**：除大纲中不存在的章外，笔记内容应尽量完整保留
8. **分层清晰**：概念→机制→影响因素→防控/应用，由浅入深
9. **表格优先**：对比性内容用 `\begin{table}[H]` + `\begin{tabularx}{\textwidth}{...}`
10. **每章附题**：名词解释 + 简答题 + 参考答案要点，覆盖该章核心知识点
11. **同时产出两个PDF**：完整版（预软）+ 精简版（预软_精简版，关闭chapterintro和review）
